...
Platforms and card BINs form a parent-child hierarchy: `/providers` browses by platform (region, KYC, tags, and BIN counts), each platform page lists all of its card BINs, and BIN pages are reached through platforms or their direct URLs. Cards support bilingual `description_zh`/`description_en` (the legacy `description` is a fallback). The homepage shows a platforms section (up to nine) and industry news; the first `/providers` page lists ceased platforms separately. Public pages are edge-cached through the Workers Cache API (10 minutes for lists, 1 hour for details) and related caches are purged automatically after admin mutations.
...
Inactive providers keep their detail URLs with prominent stopped-operating notices (HTTP 200, indexable with full hreflang, listed in the sitemap at low priority 0.3/yearly) so searches like "FomePay shut down" can find the warning; inactive card BINs show notices too but stay noindex as thin pages. Cards of inactive providers show the platform notice. Draft posts are also private.
