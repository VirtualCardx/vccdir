    ...
    <a href={langPath(lang, `/provider/${provider.slug}`)} class="card-hover group block rounded-3xl border border-slate-200/70 bg-white p-6 shadow-soft">
      <div class="mb-4 flex items-start justify-between gap-3">
        <div class="flex min-w-0 items-center gap-3">
          {provider.logo_url ? (
            <img src={`/images/${provider.logo_url}`} alt={lang === 'zh' ? `${name} Logo` : `${name} logo`} width="44" height="44" loading="lazy" class="h-11 w-11 rounded-xl object-cover" />
          ) : (
...
export function CardTile({ card, lang }: { card: CardWithProvider; lang: Lang }) {
  ...
  return (
    <a href={langPath(lang, `/card/${card.slug}`)} class="card-hover group relative block overflow-hidden rounded-3xl border border-slate-200/70 bg-white p-6 shadow-soft">
      <div class="mb-5 flex items-center justify-between gap-3">
        <div class="flex min-w-0 items-center gap-3">
          {card.provider_logo_url ? (
            <img src={`/images/${card.provider_logo_url}`} alt={lang === 'zh' ? `${name} Logo` : `${name} logo`} width="44" height="44" loading="lazy" class="h-11 w-11 rounded-xl object-cover" />
          ) : (
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-100 to-accent-50 font-bold text-brand-700 ring-1 ring-brand-100">{name.charAt(0)}</div>
          )}
          <div class="min-w-0">
      ...
      <div class="grid grid-cols-4 gap-2 rounded-2xl bg-slate-50/90 p-3 text-center ring-1 ring-slate-100">
        <div><div class="text-[11px] text-slate-400">{t('card.issuance_fee', lang)}</div><div class="mt-1 text-sm font-bold tabular-nums text-slate-900">{card.issuance_fee === 0 ? t('common.free', lang) : `$${card.issuance_fee}`}</div></div>
        <div class="border-x border-slate-200"><div class="text-[11px] text-slate-400">{t('card.fee_rate', lang)}</div><div class="mt-1 text-sm font-bold tabular-nums text-slate-900">{card.fee_rate}%</div></div>
        <div><div class="text-[11px] text-slate-400">{t('card.monthly_fee', lang)}</div><div class="mt-1 text-sm font-bold tabular-nums text-slate-900">{card.monthly_fee === 0 ? t('common.free', lang) : `$${card.monthly_fee}`}</div></div>
        <div class="border-l border-slate-200"><div class="text-[11px] text-slate-400">{t('card.currency', lang)}</div><div class="mt-1 text-sm font-bold tabular-nums text-slate-900">{card.currency}</div></div>
      </div>
